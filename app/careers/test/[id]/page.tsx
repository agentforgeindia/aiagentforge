...
type Result = {
  ...
  attempts_used: number; attempts_left: number;
  candidate_name?: string; role_slug?: string;
};
...
const ROLE_LABELS: Record<string, string> = {
  telecaller: "Telecaller", "support-executive": "Support Executive",
  "marketing-executive": "Marketing Executive", "content-creator": "Content Creator",
  "hr-executive": "HR Executive", "office-executive": "Office Executive",
};

function downloadScorePDF(result: Result, candidateId: string) {
  const date      = new Date().toLocaleDateString("en-IN", { day:"2-digit", month:"long", year:"numeric" });
  const scoreOut  = result.correct_count * 2;  // 2 marks per question
  const roleLabel = ROLE_LABELS[result.role_slug ?? ""] ?? "";
...
<head>
  ...
  <style>
    * { margin:0; padding:0; box-sizing:border-box; }
    ...
    .score-row { display:flex; align-items:baseline; justify-content:center; gap:6px; margin:16px 0 4px; }
    .score-big { font-size:72px; font-weight:900; background:linear-gradient(135deg,#06b6d4,#3b82f6); -webkit-background-clip:text; -webkit-text-fill-color:transparent; }
    .score-total { font-size:28px; font-weight:900; color:#9ca3af; }
...
<body>
  <div class="card">
    <div class="header">
      ...
      <p>Skill Test Scorecard — ${roleLabel || "General"}</p>
    </div>
    <div style="text-align:center;margin-bottom:16px;">
      <span class="badge ${result.passed?"pass":"fail"}">${result.passed?"✅ Passed":"❌ Not Passed"}</span>
    </div>
    ${result.candidate_name ? `<p style="text-align:center;font-size:18px;font-weight:900;margin-bottom:4px;">${result.candidate_name}</p>` : ""}
    <p style="text-align:center;font-size:12px;color:#6b7280;margin-bottom:8px;">
      Questions Attempted: ${result.total_questions} &nbsp;|&nbsp; Correct: ${result.correct_count} &nbsp;|&nbsp; 2 marks each
    </p>
    <div class="score-row">
      <span class="score-big">${scoreOut}</span>
      <span class="score-total">/ 100</span>
    </div>
    <p style="text-align:center;font-size:12px;color:#6b7280;margin-bottom:20px;">Pass = 60 / 100</p>
    <table class="section-table">${scores}</table>
    <div class="meta">
      <p>Attempt: ${result.attempts_used} of 3 &nbsp;|&nbsp; Trust Score: ${result.trust_score}%</p>
      <p style="margin-top:4px;">Date: ${date}</p>
      <p style="margin-top:4px;">Candidate ID: ${candidateId}</p>
      <p style="margin-top:12px;font-size:10px;">Auto-generated by AgentForge recruitment system · agentforge.in</p>
    </div>
  </div>
</body>
</html>`;
...
export default function TestPage() {
      ...
      <Shell>
        <div className="py-6 text-center">
          ...
          <p className="mt-2 text-sm font-medium text-black/60 dark:text-white/60">
            Thank you for applying to AgentForge. Our team will review your application and contact you shortly.
          </p>
          <p className="mt-4 text-[11px] font-bold text-black/40 dark:text-white/40">You may close this window.</p>
        </div>
      </Shell>
    );
  }
        ...
        {isStrong ? (
          <>
            <div className="mt-5 rounded-2xl border border-purple-200/60 bg-purple-50/60 p-4 dark:border-purple-400/20 dark:bg-purple-500/5">
              ...
              <p className="mt-2 text-sm font-medium leading-relaxed text-black/70 dark:text-white/70">
                Your social media reach looks strong. We are looking for referral-based collaborators.
              </p>
              <ul className="mt-3 space-y-1.5 text-sm font-medium text-black/65 dark:text-white/65">
                <li>🔗 You will receive a unique referral link</li>
                <li>💰 <b>10% commission</b> on every purchase</li>
          ...
          <div className="mt-5 rounded-2xl border border-amber-200/60 bg-amber-50/60 p-4 dark:border-amber-400/20 dark:bg-amber-500/5 text-center">
            <p className="text-sm font-medium text-black/70 dark:text-white/70">
              Thank you for applying! Our social media team will review your profile and reach out to you shortly.
            </p>
  ...
  if (phase === "result" && result) {
    const marksEarned = result.correct_count * 2;
    const marksTotal  = result.total_questions * 2;
    const learnHref   = `/careers/learn?role=${result.role_slug ?? "telecaller"}&cid=${id}`;
      ...
      <Shell>
        <div className="text-center">
          ...
          {result.candidate_name && (
            <p className="mt-1 text-sm font-bold text-slate-600 dark:text-slate-300">{result.candidate_name}</p>
          )}

          {/* Marks display — X correct × 2 = score / total */}
          <div className="mt-5 inline-block rounded-2xl border border-slate-200 bg-slate-50 px-6 py-4 dark:border-slate-700 dark:bg-slate-800/50">
            <div className="inline-flex items-baseline gap-1">
              <span className="bg-gradient-to-r from-cyan-500 to-blue-600 bg-clip-text text-6xl font-black text-transparent">{marksEarned}</span>
              <span className="text-xl text-black/40 dark:text-white/40">/ {marksTotal}</span>
            </div>
            <p className="mt-1 text-[11px] text-slate-500">
              {result.correct_count} correct × 2 marks &nbsp;·&nbsp; Pass = {Math.round(marksTotal * 0.6)} / {marksTotal}
            </p>
          </div>
          ...
          <div className="mt-5 rounded-2xl border border-amber-200/60 bg-amber-50/60 p-4 text-center dark:border-amber-400/20 dark:bg-amber-500/5">
            <p className="text-sm font-black text-amber-800 dark:text-amber-200">
              {result.attempts_left > 0
                ? `${result.attempts_left} attempt${result.attempts_left > 1 ? "s" : ""} remaining (${result.attempts_used} of 3 used)`
                : "All 3 attempts used — test locked."}
            </p>
            {result.attempts_left > 0 && (
              <>
                <p className="mt-1 text-[11px] font-medium text-black/60 dark:text-white/50">
                  Review the training material and try again — it will be easier!
                </p>
                <a href={learnHref}
                  className="mt-3 inline-flex items-center gap-1.5 rounded-full bg-amber-500 px-5 py-2 text-xs font-black text-white shadow transition hover:scale-[1.02] hover:bg-amber-600">
                  Review Training →
                </a>
              </>
            )}
        ...
        <p className="mt-5 text-center text-[11px] text-slate-400">Trust score: {result.trust_score}% · Attempt {result.attempts_used}/3 · ID: {id.slice(0,8)}</p>
      </Shell>
    );
  }
