...
// Blog dropdown — category quick-links + 3 most-recent posts (computed once)
const blogCategories = [
  { label: "Textile", desc: "Mockups, catalogue & saree guides", emoji: "👕", href: "/blog" },
  { label: "Jewellery", desc: "Bridal & daily-wear photography", emoji: "💎", href: "/blog" },
  { label: "Productography", desc: "D2C product shoots & ad creative", emoji: "📸", href: "/blog" },
];
...
export default function Navbar() {
  ...
  useEffect(() => {
    function handleClick(e: MouseEvent) {
      ...
      if (blogRef.current && !blogRef.current.contains(e.target as Node)) setShowBlog(false);
...
>
  <div className="flex h-10 w-10 shrink-0 items-center justify-center overflow-hidden rounded-xl shadow-lg transition-transform duration-200 group-hover:scale-105 sm:h-11 sm:w-11">
    <img
  src="/af-logo.png"
  alt="AgentForge AI logo — AI textile mockup, jewellery photoshoot & product photography studio India"
  width={44}
  height={44}
  loading="eager"
  fetchPriority="high"
  ...
  darkMode ? "border-white/10 bg-[#0b1220]/98" : "border-black/10 bg-white/98"
          ...
          <div className="relative" ref={blogRef}>
            ...
            >
              Blog
              <svg
                className={`h-3.5 w-3.5 transition-transform duration-200 ${showBlog ? "rotate-180" : ""}`}
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
                strokeWidth={2.5}
            ...
            {showBlog && (
              <div
                className={`absolute right-0 top-14 z-[9999] w-[22rem] overflow-hidden rounded-[2rem] border shadow-2xl backdrop-blur-2xl ${
                  darkMode ? "border-white/10 bg-[#0b1220]/98" : "border-black/10 bg-white/98"
                }`}
              >
                {/* Category quick-links */}
                <div className={`grid grid-cols-3 gap-1 border-b p-2 ${darkMode ? "border-white/10" : "border-black/10"}`}>
                  {blogCategories.map((cat) => (
                    <Link
                      key={cat.label}
                      href={cat.href}
                      className={`flex flex-col items-center gap-1 rounded-2xl px-2 py-3 text-center transition ${
                        darkMode ? "hover:bg-white/8" : "hover:bg-cyan-50"
                      }`}
                    >
                ...
                {/* Recent posts */}
                <div className="p-1">
                  <p className={`px-3 pb-1 pt-2 text-[10px] font-black uppercase tracking-[0.22em] ${muted}`}>
                    Latest articles
                  </p>
                  {recentBlogPosts.map((post) => (
                    ...
                    >
                      <span className="text-xl">{post.heroEmoji}</span>
                      <div className="min-w-0 flex-1">
                ...
                <Link
                  href="/blog"
                  className={`block border-t px-4 py-3 text-center text-xs font-black text-cyan-600 transition dark:text-cyan-300 ${
                    darkMode ? "border-white/10 hover:bg-white/8" : "border-black/10 hover:bg-cyan-50"
                  }`}
                >
                  Browse all articles →
                </Link>
