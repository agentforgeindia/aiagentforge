...
export default function Home() {
  ...
  const generateOne = async (item: GenItem, userId: string): Promise<void> => {
    ...
    const modelPhotoInstruction = modelPhotoUrl
      ? isHomeLikeCategory
        ? `UPLOADED MODEL (see model_photo_url): Place the EXACT person from this uploaded photo INTO the scene with the product (${resolvedSudanGarment || resolvedProduct || "the product"}). The person should naturally interact with the product — standing beside / holding / pointing toward it as per the selected interaction. Preserve their face, identity, body shape and skin tone exactly — do not change the face. The person is a MODEL posing with the home-textile product; they do NOT wear it. CRITICAL SCALE: size the person REALISTICALLY for the room — correct real-world human proportions relative to the ${resolvedSudanGarment || resolvedProduct || "product"}, walls, floor and furniture, with natural perspective, depth and grounded shadows. Feet firmly on the floor, full body in frame, standing at a natural distance. The person must NOT look oversized, zoomed-in or larger than the scene — they should blend in as if really photographed in that room. Realistic, tasteful, fully clothed. Absolutely no nudity, no sexualisation and no inappropriate content.`
