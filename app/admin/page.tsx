          ...
          {grouped.map((section) => (
            <section key={section.group}>
              <div className="mb-3 flex items-center gap-2.5">
                <span className={`inline-flex h-7 w-7 items-center justify-center rounded-lg bg-gradient-to-br ${accentFor(section.group)} text-white shadow-sm`}>
                  {GROUP_ICON[section.group] ?? <Crosshair className="h-4 w-4" />}
                </span>
                <h2 className="text-sm font-black tracking-tight">{section.group}</h2>
                <span className="rounded-full bg-slate-100 px-2 py-0.5 text-[10px] font-bold text-slate-400 dark:bg-slate-800">
                  {section.tiles.length} modules
                </span>
                ...
                {section.tiles.map((t) => (
                  <Link key={t.href} href={t.href} className="group block">
                    <div className="relative flex h-full items-start gap-3 overflow-hidden rounded-xl border border-slate-200/80 bg-white p-3.5 shadow-[0_1px_2px_rgba(0,0,0,0.04),0_10px_24px_-16px_rgba(0,0,0,0.18)] transition-all duration-300 hover:-translate-y-1 hover:border-cyan-300 hover:shadow-[0_10px_30px_-10px_rgba(34,211,238,0.38)] dark:border-slate-800 dark:bg-[#0e1117] dark:hover:border-cyan-500/40">
                      ...
                      <span className={`relative inline-flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br ${accentFor(section.group)} text-white shadow-lg shadow-cyan-500/20 ring-1 ring-white/25 transition-transform duration-300 group-hover:scale-110 group-hover:-rotate-3`}>
                        {t.icon}
                      </span>
